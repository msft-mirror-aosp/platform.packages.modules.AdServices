...
public final class AsyncSourceFetcherTest extends AdServicesExtendedMockitoTestCase {
    ...
    @Mock private CountUniqueRegistrar mCountUniqueRegistrar;
    ...
    public void fetchWebSources_basic_success() throws IOException {
        ...
        when(mUrlConnection.getResponseCode()).thenReturn(200);
        when(mUrlConnection.getURL()).thenReturn(new URL(REGISTRATION_URI_1.toString()));
        when(mUrlConnection.getHeaderFields())
                .thenReturn(
                        Map.of(
                                "Attribution-Reporting-Register-Source",
    ...
    public void fetchSource_setsRegistrationOriginWithoutPath_forRegistrationURIWithPath()
        ...
        when(mUrlConnection.getResponseCode()).thenReturn(200);
        when(mUrlConnection.getURL()).thenReturn(new URL(uri));
        when(mUrlConnection.getHeaderFields())
                .thenReturn(
                        Map.of(
                                "Attribution-Reporting-Register-Source",
    ...
    public void fetchSource_flexibleEventReportApi_valid() throws Exception {
        ...
        List<Long> expectedWindows = List.of(
    ...
    @Test
    public void fetchSource_flexEventSourceTypeEventNoWindows_expiryBoundsWindows()
            throws Exception {
        String triggerSpecsString =
                "[{\"trigger_data\": [0, 1, 2],"
                        + "\"summary_operator\": \"count\", "
    ...
    @Test
    public void fetchSource_modulusTopLevelTriggerDataMatchingNonContiguous_fail()
            throws Exception {
        RegistrationRequest request =
                buildDefaultRegistrationRequestBuilder(DEFAULT_REGISTRATION).build();
        doReturn(mUrlConnection).when(mFetcher).openUrl(new URL(DEFAULT_REGISTRATION));
    ...
    @Test
    public void fetchSource_doesNotRegistersCountUniqueEvent_IfAppNotInAllowList()
            throws IOException {
    ...
    public void fetchSource_registersCountUniqueMetadata_IfCountUniqueMetadataHeaderPresent()
        ...
        when(mUrlConnection.getHeaderFields())
                .thenReturn(
                        Map.of(
                                ...
                                List.of(
                                        "set;key=\"key1\";value=\"1\";ignore_if_present, set;"
                                                + "key=\"key2\";value=\"2\", \n"
                                                + "delete;key=\"key1\""),
                                "Attribution-Reporting-Register-Source",
                                List.of(
        ...
        // Assertion
        verify(mCountUniqueRegistrar).registerCountUniqueMetadata(any(), any());
        assertThat(asyncFetchStatus.getResponseStatus())
                .isEqualTo(AsyncFetchStatus.ResponseStatus.SUCCESS);
        assertThat(fetch.isPresent()).isTrue();
    }
