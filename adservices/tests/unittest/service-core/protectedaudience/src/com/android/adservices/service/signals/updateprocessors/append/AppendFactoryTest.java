...
public class AppendFactoryTest extends AdServicesMockitoTestCase {
    ...
    private AppendFactory mAppendFactory;

    @Before
    public void setup() {
        when(mEvictionPriorityHandlerFactoryMock.getHandler())
                .thenReturn(new EvictionPriorityHandlerNoOpImpl());
        mAppendFactory = new AppendFactory(mEvictionPriorityHandlerFactoryMock);
    }

    @Test
    public void testGetUpdateProcessor_supportedVersions() {
        expect.withMessage("v0 processor")
                .that(mAppendFactory.getUpdateProcessor(UpdateSchemaVersion.V0))
                .isInstanceOf(AppendV0.class);

        expect.withMessage("v1 processor")
                .that(mAppendFactory.getUpdateProcessor(UpdateSchemaVersion.V1))
                .isInstanceOf(AppendV1.class);
    }
    ...
    public void testGetUpdateProcessor_unsupportedVersion() {
        assertThrows(
                ...
                () -> mAppendFactory.getUpdateProcessor(UNSUPPORTED_VERSION));
    }
}
