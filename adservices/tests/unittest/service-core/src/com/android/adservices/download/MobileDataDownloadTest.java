 ...
 *
 * <p>When new MDD download file links are added, the file should be downloaded and placed inside
 * assets/mobiledatadownload. This test will use a local copy of the file instead of making network
 * calls, as network calls were flaky.
 */
...
public final class MobileDataDownloadTest extends AdServicesExtendedMockitoTestCase {
    ...
    private static final String UI_OTA_STRINGS_MANIFEST_FILE_URL =
            "https://www.gstatic.com/mdi-serving/rubidium-adservices-ui-ota-strings/4265/91ac5d17ab9f2445a3462f98f5249a73e9b0e235";
    ...
    private static final String MDD_PACKAGE_DENY_MANIFEST_FILE_URL =
            "https://www.gstatic.com/mdi-serving/rubidium-adservices-package-deny-list/7769"
                    + "/760d5af3820ff94b9c5cd504a8628ae134dc3917";

    private static final int ARGON_CONFIG_BUILD_ID = 9828;
    private static final MddConfigs argonMddConfigs =
            MddConfigs.newBuilder()
                    .addMddConfigs(
                            MddConfigs.MddConfig.newBuilder()
                                    .setManifestId("enrollment")
                                    .setManifestUrl(
                                            "https://www.gstatic.com/mdi-serving/rubidium-adservices-enrollment/"
                                                    + ARGON_CONFIG_BUILD_ID
                                                    + "/f8d5eefa2016ce42da1c6be6ca843f902c46c3ca")
                                    .build())
                    .build();
    ...
    public static final String ARGON_CONFIG_FILE_GROUP_NAME_1 = "rubidium_adservices_enrollment";

    private static Supplier<FileDownloader> fileDownloaderSupplier;
    ...
    public void testUiOtaStringsManifestFileGroupPopulator()
        ...
        // Verify mdd downloaded file group.
        assertWithMessage(
                        "No group exists or has been downloaded for %s group",
                        UI_OTA_STRINGS_FILE_GROUP_NAME)
                .that(clientFileGroup)
                .isNotNull();
        ...
        expect.that(clientFileGroup.getStatus()).isEqualTo(ClientFileGroup.Status.DOWNLOADED);
        expect.that(clientFileGroup.getBuildId()).isEqualTo(4265); // Build ID generated by Ingress
    }
    ...
    public void testPackageDenyManifestFileGroupPopulator_featureEnabled() throws Exception {
        ...
        expect.that(clientFileGroup.getStatus()).isEqualTo(ClientFileGroup.Status.DOWNLOADED);
        expect.that(clientFileGroup.getBuildId())
                .isEqualTo(/* BuildID generated by Ingress */ 7769);
    }
    ...
    public void testArgonConfigDataDownload_preConsentAllowed() throws Exception {
        ...
        ClientFileGroup clientFileGroup =
                mMdd.getFileGroup(
                                GetFileGroupRequest.newBuilder()
                                        .setGroupName(ARGON_CONFIG_FILE_GROUP_NAME_1)
                                        .build())
                        .get();

        // Verify argon config file group.
        expect.that(clientFileGroup.getGroupName()).isEqualTo(ARGON_CONFIG_FILE_GROUP_NAME_1);
        expect.that(clientFileGroup.getOwnerPackage()).isEqualTo(mContext.getPackageName());
        expect.that(clientFileGroup.getFileCount()).isEqualTo(1);
        expect.that(clientFileGroup.getStatus()).isEqualTo(ClientFileGroup.Status.DOWNLOADED);
        expect.that(clientFileGroup.getBuildId()).isEqualTo(ARGON_CONFIG_BUILD_ID);
    }
    ...
    @Test
    public void testArgonConfigDataDownload_emptyUrl() throws Exception {
        when(mMockFlags.getConfigDeliveryMddConfigs())
                .thenReturn(
                        MddConfigs.newBuilder()
                                .addMddConfigs(
                                        MddConfigs.MddConfig.newBuilder()
                                                ...
                                                .setManifestUrl("")
                                                .setIsDownloadPreConsent(true)
                                                .addFileGroupNames("argon-config-empty")
                                                .build())
                                .build());
        createMddForArgonConfig();
        ...
        ClientFileGroup clientFileGroup =
                mMdd.getFileGroup(
                                GetFileGroupRequest.newBuilder()
                                        .setGroupName("argon-config-empty")
                                        .build())
                        .get();
    ...
    @Test
    public void testArgonConfigDataDownload_emptyManifestId() throws Exception {
        when(mMockFlags.getConfigDeliveryMddConfigs())
                .thenReturn(
                        MddConfigs.newBuilder()
                                .addMddConfigs(
                                        MddConfigs.MddConfig.newBuilder()
                                                .setManifestUrl("https://dummy.com")
                                                .build())
                                .build());
        createMddForArgonConfig();
    ...
    @Test
    public void testArgonConfigDataDownload_emptyMddConfigs() throws Exception {
        when(mMockFlags.getConfigDeliveryMddConfigs()).thenReturn(MddConfigs.newBuilder().build());

        createMddForArgonConfig();

        // MDD should not crash when MddConfigs is empty.
    }

    @Test
    public void testArgonConfigDataDownload_consentRevoked() throws Exception {
        when(mMockFlags.getConfigDeliveryMddConfigs()).thenReturn(argonMddConfigs);
        when(mMockFlags.getGaUxFeatureEnabled()).thenReturn(true);
        when(mConsentManager.getConsent(AdServicesApiType.MEASUREMENTS))
                .thenReturn(AdServicesApiConsent.REVOKED);
        ...
        fileDownloaderSupplier =
                () ->
                        new TestFileDownloader(
                                fileStorage, MoreExecutors.listeningDecorator(DOWNLOAD_EXECUTOR));
    ...
    // Returns MobileDataDownload using passed in Argon config manifest url.
    private void createMddForArgonConfig() throws Exception {
        mMdd =
                getMddForTesting(
                        mContext,
                        mMockFlags,
                        MobileDataDownloadFactory.getArgonManifestPopulators(
                                mMockFlags, mFileStorage, mFileDownloader));
