...
public class E2ETriggerNoiseMockTest extends E2EMockTest {
    ...
    public E2ETriggerNoiseMockTest(Collection<Action> actions, ReportObjects expectedOutput,
        ...
        mAttributionHelper = TestObjectProvider.getAttributionJobHandler(
                TestObjectProvider.Type.NOISY, sDatastoreManager);
    ...
    private boolean expectedAndActualCountsAreNotEqual() {
        int expectedCount = 0;
        for (Map<String, Integer> expectedDistribution
                : mExpectedTriggerDataDistributions.values()) {
            expectedCount += expectedDistribution.values().stream().reduce(0, Integer::sum);
        }
        int actualCount = 0;
        for (Map<String, Integer> actualDistribution
                : mActualTriggerDataDistributions.values()) {
            actualCount += actualDistribution.values().stream().reduce(0, Integer::sum);
        }
