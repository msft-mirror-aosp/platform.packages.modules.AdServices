...
public final class CustomAudienceShellCommandsTest extends FledgeScenarioTest {
    ...
    private AdServicesShellCommandHelper mShellCommandHelper;

    @Before
    public void setupCustomAudienceShellCommandsScenarioTest() {
        mCustomAudienceTestFixture = new CustomAudienceTestFixture(sContext);
        mShellCommandHelper = new AdServicesShellCommandHelper();
    }

    @Test
    public void testRun_listCustomAudience_happyPath() throws Exception {
        CustomAudience shirtsCustomAudience =
        ...
        CustomAudience shoesCustomAudience =
                mCustomAudienceTestFixture.createCustomAudience(
                        SHOES_CA,
                        BUYER,
                        List.of(1D),
        ...
        joinCustomAudience(shirtsCustomAudience);
        joinCustomAudience(shoesCustomAudience);
        ...
        CustomAudienceSubject.assertThat(customAudience1).hasValidActivationTime();
        CustomAudienceSubject.assertThat(customAudience1).hasValidationFailures(0);
        CustomAudienceSubject.assertThat(customAudience1).hasTimeoutFailures(0);
        CustomAudienceSubject.assertThat(customAudience2).hasValidActivationTime();
        CustomAudienceSubject.assertThat(customAudience2).hasValidationFailures(0);
        CustomAudienceSubject.assertThat(customAudience2).hasTimeoutFailures(0);
        CustomAudienceSubject.assertThat(customAudience3).hasValidActivationTime();
        CustomAudienceSubject.assertThat(customAudience3).hasValidationFailures(0);
        CustomAudienceSubject.assertThat(customAudience3).hasTimeoutFailures(0);
    }
    ...
    public void testRun_viewCustomAudience_happyPath() throws Exception {
        ...
        JSONObject customAudience =
                runAndParseShellCommandJson(
                        ...
                        OWNER, BUYER, shirtsCustomAudience.getName());
        leaveCustomAudience(shirtsCustomAudience);

        CustomAudience parsedCustomAudience = fromJson(customAudience);
        assertThat(shirtsCustomAudience).isEqualTo(parsedCustomAudience);
        CustomAudienceSubject.assertThat(customAudience).hasValidActivationTime();
        CustomAudienceSubject.assertThat(customAudience).hasValidationFailures(0);
        CustomAudienceSubject.assertThat(customAudience).hasTimeoutFailures(0);
    }

    @Test
    public void testRun_refreshCustomAudiences_verifyNoCustomAudienceChanged() {
        AbstractAdServicesShellCommandHelper.CommandResult commandResult =
                mShellCommandHelper.runCommandRwe(
                        "custom-audience refresh --owner %s --buyer %s --name %s",
                        OWNER, BUYER, SHIRTS_CA);
    ...
    public void testRun_refreshCustomAudiences_verifyCustomAudienceChanged() throws Exception {
        ...
        CustomAudience customAudienceBefore = getCustomAudience(adTechIdentifier, SHOES_CA);
        mShellCommandHelper.runCommand(
                "custom-audience refresh --owner %s --buyer %s --name %s",
                OWNER, adTechIdentifier, SHOES_CA);
        CustomAudience customAudienceAfter = getCustomAudience(adTechIdentifier, SHOES_CA);
    ...
    private CustomAudience getCustomAudience(AdTechIdentifier adTechIdentifier, String name)
            throws JSONException {
        return CustomAudienceShellCommandHelper.fromJson(
                runAndParseShellCommandJson(
                        "custom-audience view --owner %s --buyer %s --name %s",
                        OWNER, adTechIdentifier, name));
    }
