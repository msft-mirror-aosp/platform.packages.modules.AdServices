...
public interface EncoderLogicDao {
     ...
     */
    @Query("SELECT * FROM encoder_logics WHERE buyer = :buyer")
     ...
     */
    @Query("DELETE FROM encoder_logics WHERE buyer = :buyer")
    void deleteEncoder(AdTechIdentifier buyer);

    /** Deletes all persisted encoding logics */
    @Query("DELETE FROM encoder_logics")
